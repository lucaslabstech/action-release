...
const INCLUDE_COMIT_AUTHOR = getInput('include-commit-author') === 'true';

console.log('START_FROM: ', START_FROM);
console.log('END_TO: ', END_TO);
console.log('INCLUDE_COMMIT_BODY: ', INCLUDE_COMMIT_BODY);
console.log('INCLUDE_ABBREVIATED_COMMIT: ', INCLUDE_ABBREVIATED_COMMIT);
console.log('INCLUDE_COMIT_AUTHOR: ', INCLUDE_COMIT_AUTHOR);
...
if (INCLUDE_COMIT_AUTHOR) {
    commitDataMap.set('author', '%an');
}
...
function getCommits() {
    ...
    const command = `git --no-pager log ${START_FROM}..${END_TO} --pretty=format:"${format}" --reverse`;
    console.log('getting commits with command:\n', command);
    const logs = String(execSync(command));
    console.log('gotten logs: ', logs);
...
function getCommitsList(commits, pad = '') {
    ...
    for (const commit of commits) {
        ...
        if(commit.author !== undefined && commit.abbreviated_commit !== undefined) {
            changelog += ` - by ${commit.author} in ${commit.abbreviated_commit}`;
        } else if (commit.author !== undefined) {
            changelog += ` - by ${commit.author}`;
        } else if (commit.abbreviated_commit !== undefined) {
...
function getChangeLog(groups) {
    ...
    for (const [typeId, group] of groups) {
        if (group.isEmpty || typeId === 'release' || typeId === 'rel') {
            continue;
        }
